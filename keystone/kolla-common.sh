...
check_for_db() {
    ...
    check_required_vars DB_SERVICE_HOST DB_ROOT_PASSWORD

    mysql -h ${DB_SERVICE_HOST} -u root -p"${DB_ROOT_PASSWORD}" \
            -e "select 1" $database > /dev/null 2>&1 || {
        echo "ERROR: database $database is not available @ $DB_SERVICE_HOST" >&2
        return 1
    }

    echo "database is active @ ${DB_SERVICE_HOST}"
}
